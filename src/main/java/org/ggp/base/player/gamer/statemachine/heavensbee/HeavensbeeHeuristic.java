...
public final class HeavensbeeHeuristic extends SampleGamer {
	private Logger log = null;
	private int depth_limit = 0;
	private long hard_deadline;
	private int max_game_actions;

	public HeavensbeeHeuristic() throws SecurityException, IOException {
...
//	    log.log(Level.INFO, "Starting instance of {0}", ClassName.class.getName());
	...
	public int maxscore(Role role, MachineState state, int depth) throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException
	{
		...
		{
			int cur_mobility = alquerque(role, state);
			log.log( Level.FINE, "{0} is >= depth limit {1}; mobility score is {2}", new Object[]{depth, depth_limit, cur_mobility} );
			return cur_mobility;
		}
	...
	{
		log.log( Level.FINE, "Starting mp_min_score with depth {0}", new Object[]{depth} );
			...
			throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException
	{
		...
		log.log( Level.FINE, "Starting mp_max_score with depth {0}: time {1} deadline {2}", new Object[]{depth, now, hard_deadline} );
	...
	public int oppmobility(Role role, Role opponent, MachineState state) throws MoveDefinitionException, GoalDefinitionException
	{
		StateMachine sm = getStateMachine();
		List<Move> actions = sm.getLegalMoves(state, opponent);

		return 50 - actions.size();

		//return (5 * actions.size() / max_game_actions);
	}

	public int mobility(Role role, MachineState state) throws MoveDefinitionException, GoalDefinitionException
	{
		StateMachine sm = getStateMachine();
		List<Move> actions = sm.getLegalMoves(state, role);
		return (5 * actions.size() / max_game_actions);
	...
	public int alquerque(Role role, MachineState state) throws MoveDefinitionException, GoalDefinitionException
	{
		StateMachine sm = getStateMachine();
		return sm.getGoal(state, role);
